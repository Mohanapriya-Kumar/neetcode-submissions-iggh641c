class Solution {
    public String minWindow(String s, String t) {
        ...
        int l = 0;
        int reslen = Integer.MAX_VALUE; //important reslen is initialised to infinity
        int[] res = {-1,-1}; // initially it cannot be zero but -1..
        int have = 0, need = tc.size(); // we need just the unique..
        for(int r = 0; r < s.length(); r++){
            char c = s.charAt(r);
            wc.put(c, wc.getOrDefault(c, 0) + 1);
            if(tc.containsKey(c) && wc.get(c) == tc.get(c)) have++;
            while(have == need){
                if((r - l + 1) < reslen){
                    reslen = r - l + 1;
                    res[0] = l;
